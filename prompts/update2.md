...
## Context
We have an admin tool repository that already manages some setup for our robot-secret broker (“xpass”). We now want it to:
...
Given a KV v2 mount `kv_mount` and prefix `secret_prefix` (example `projects` + `xpass`):
...
Vault security:
- For each bot `<bot_name>`, create a dedicated Vault policy `xpass-bot-<bot_name>` granting:
  - read/list bot-private subtree for that bot
  - read/list shared subtree
- Create a shared-only policy `xpass-shared-read` (read/list shared subtree)
- Create a Vault OIDC role per bot named exactly `<bot_name>` (or configurable prefix), under `auth/<oidc_mount>/role/<role>`
  ...
  - The role attaches only policies:
    - `xpass-bot-<bot_name>` (which already includes shared)
  - The role sets:
