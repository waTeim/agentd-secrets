# Prompt: Rebuild x-pass broker as Node.js + Playwright (Keycloak IdP, Duo in Browser flow)

You are a senior security engineer and Node.js backend developer. Re-implement the existing “x-pass” secret broker as a wholly Node.js service that uses a headless browser (Playwright) to obtain a Keycloak user token via OIDC Authorization Code + PKCE, rather than UMA/RPT polling. Maintain compatibility with the existing Helm chart/env/secret/config expectations described below.
...
The current x-pass broker sits between an automated client (bot) and HashiCorp Vault. It enforces “human-in-the-loop approval” by relying on Keycloak as the authorization gate. It never stores actual secrets; it only returns Vault response-wrapping tokens, encrypted at rest with AES-256-GCM using a 32-byte key in `WRAPTOKEN_ENC_KEY` (hex). It exposes:
...
Read service registry YAML from `/etc/x-pass/config.yaml` (as mounted by the Helm chart).
...
Implement fully—no TODO placeholders. Keep the external contract (endpoints, env var names, service registry path) compatible with the existing x-pass deployment described above, and keep the Helm chart in `./helm/` as the deployment contract.
