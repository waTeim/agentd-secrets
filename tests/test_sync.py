"""Tests for the xpass-admin sync subcommand."""
...
import importlib
xpass_admin = importlib.import_module("xpass-admin")
...
class TestPolicyHCLBuilders:
    ...
    def test_shared_policy_hcl(self):
        hcl = xpass_admin.build_shared_policy_hcl("projects", "xpass")
        assert 'path "projects/data/xpass/shared/*"' in hcl
        assert '"read"' in hcl
        assert 'path "projects/metadata/xpass/shared/*"' in hcl
        assert '"list"' in hcl
        # Should NOT contain bot paths
        assert "bots" not in hcl

    def test_bot_policy_hcl(self):
        hcl = xpass_admin.build_bot_policy_hcl("projects", "xpass", "openclaw")
        # Bot-specific paths
        assert 'path "projects/data/xpass/bots/openclaw/*"' in hcl
        assert 'path "projects/metadata/xpass/bots/openclaw/*"' in hcl
    ...
    def test_bot_policy_isolation(self):
        ...
        hcl_a = xpass_admin.build_bot_policy_hcl("projects", "xpass", "openclaw")
        hcl_b = xpass_admin.build_bot_policy_hcl("projects", "xpass", "roadrunner")
...
class TestFreshInstall:
    ...
    def test_fresh_install_apply(self):
        ...
        assert "xpass-shared-read" in state["policies"]
        shared_hcl = state["policies"]["xpass-shared-read"]
        assert "shared" in shared_hcl

        # Bot policies were created
        assert "xpass-bot-openclaw" in state["policies"]
        assert "xpass-bot-roadrunner" in state["policies"]
        ...
        assert openclaw_role["policies"] == ["xpass-bot-openclaw"]
        ...
        assert roadrunner_role["policies"] == ["xpass-bot-roadrunner"]
...
class TestDriftDetection:
    ...
    def test_drift_check_exits_2(self):
        ...
        state["policies"]["xpass-shared-read"] = "# tampered policy"
        ...
        assert any(i.name == "xpass-shared-read" for i in drift_items)
...
class TestDriftApply:
    ...
    def test_drift_apply_fixes(self):
        ...
        state["policies"]["xpass-bot-openclaw"] = "# tampered"
        ...
        assert "tampered" not in state["policies"]["xpass-bot-openclaw"]
        assert "openclaw" in state["policies"]["xpass-bot-openclaw"]
...
class TestMultiBotIsolation:
    ...
    def test_bot_policy_no_cross_access(self):
        ...
        openclaw_hcl = state["policies"]["xpass-bot-openclaw"]
        roadrunner_hcl = state["policies"]["xpass-bot-roadrunner"]
